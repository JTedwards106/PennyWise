    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto p-6">
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {CONFIG.SUPPORTED_RETAILERS.map(r => (
                      <label key={r.name} className="flex items-center space-x-2 p-3 border rounded-lg cursor-pointer">
                        <input type="checkbox" checked={preferences.preferredRetailers?.includes(r.name)} onChange={() => toggleRetailer(r.name)} className="w-4 h-4" />
                        <span>{r.name}</span>
                      </label>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <label className="flex items-center space-x-3 p-3 border rounded-lg cursor-pointer">
                    <input type="checkbox" checked={preferences.notificationsEnabled} onChange={(e) => updatePreference('notificationsEnabled', e.target.checked)} className="w-4 h-4" />
                    <span>Enable price drop notifications</span>
                  </label>
                  <div className="mt-4">
                    <label className="block text-sm font-medium mb-2">Price Drop Threshold (%)</label>
                    <input type="number" value={preferences.priceDropThreshold} onChange={(e) => updatePreference('priceDropThreshold', parseInt(e.target.value) || 10)} className="w-full px-4 py-2 border rounded-lg" />
                  </div>
                </div>
              </div>
            )}
                ...
                <div>
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4">
                    ...
                    <ol className="text-sm space-y-1 list-decimal list-inside">
                      <li>Visit <a href="https://makersuite.google.com/app/apikey" target="_blank" className="underline">Google AI Studio</a></li>
                      <li>Sign in and click "Get API Key"</li>
                      <li>Copy and paste below</li>
                    </ol>
                  </div>
                  <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="Enter Gemini API key" className="w-full px-4 py-2 border rounded-lg font-mono text-sm" />
            ...
            {activeTab === 'tracked' && (
              <div className="space-y-4">
                <div className="flex justify-between">
                  ...
                  {trackedProducts.length > 0 && <button onClick={clearAllTracked} className="text-sm text-red-600 hover:bg-red-50 px-4 py-2 rounded">Clear All</button>}
                </div>
                {trackedProducts.length === 0 ? (
                  <div className="text-center py-12 text-gray-500">No products tracked yet</div>
                ) : (
                  <div className="space-y-3">
                    {trackedProducts.map((p, i) => (
                      <div key={i} className="border rounded-lg p-4 flex gap-4">
                        {p.imageUrl && <img src={p.imageUrl} alt={p.title} className="w-16 h-16 object-contain" />}
                        <div className="flex-1">
                          <h4 className="font-medium text-sm line-clamp-2">{p.title}</h4>
                          <p className="text-xs text-gray-500">{p.retailer}</p>
                          <p className="text-lg font-bold text-primary-600 mt-1">${p.price.toFixed(2)}</p>
                        </div>
                        <button onClick={() => removeTrackedProduct(i)} className="text-sm text-red-600 hover:bg-red-50 px-3 py-1 rounded self-start">Remove</button>
                      </div>
                    ))}
                  </div>
                )}
        ...
        <div className="flex justify-end">
          <button onClick={handleSave} className="px-8 py-3 bg-primary-600 text-white rounded-lg hover:bg-primary-700 font-medium shadow-md">Save Settings</button>
        </div>
      </div>
    </div>
  );
